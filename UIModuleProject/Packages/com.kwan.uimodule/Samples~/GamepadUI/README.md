...
- 방향키 또는 게임패드 D-pad/스틱으로 Button과 Slider를 이동하고 Submit으로 Popup을 엽니다.
- 첫 Popup의 **Open Nested Popup**을 선택해 두 단계 Popup을 열 수 있습니다.
- Cancel을 한 번 누를 때마다 한 단계만 닫히며, 기존 선택이 순서대로 복원됩니다.

이 예제는 프리팹 없이 동적 UI를 생성하므로 pooling을 끕니다. 실제 프로젝트에서는 `Resources` 프리팹과 UI Dashboard 워크플로를 그대로 사용해도 됩니다.

## 선택 설정 흐름

`CustomUI.inputactions`에는 비표준 이름의 `MoveSelection`, `ConfirmSelection`, `DismissPanel` 액션이 있습니다. `UIInputConfiguration` 에셋의 Navigate, Submit, Cancel에 각각 연결한 뒤 `OptionalInputConfigurationSample`의 **Input Configuration**에 할당하세요.

선택 설정은 표준 `UI/*` 액션을 쓰지 않는 프로젝트에만 필요합니다. 두 Bootstrap은 각각 UIManager를 만들므로 같은 Scene에 함께 배치하지 마세요.
